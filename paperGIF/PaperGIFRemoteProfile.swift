...
enum PaperGIFOpenBuildsUnits: String, Codable, Sendable {
    case millimeters = "mm"
    case inches = "in"
}
...
struct PaperGIFOpenBuildsController: Codable, Equatable, Sendable {
    ...
    init(
        ...
        jogSpeed: Int = 1_000,
        jogMode: PaperGIFRemoteJogMode = .incremental,
        units: PaperGIFOpenBuildsUnits = .millimeters,
    ...
    init(from decoder: Decoder) throws {
        ...
        jogMode = try container.decodeIfPresent(PaperGIFRemoteJogMode.self, forKey: .jogMode) ?? .incremental
        units = try container.decodeIfPresent(PaperGIFOpenBuildsUnits.self, forKey: .units) ?? .millimeters
